...
def _pack_short_name(user_id: int, bot_username: str, packnum: int = 0) -> str:
    """Build a Telegram-valid sticker pack short name (max 64 chars)."""
    ...
    prefix = prefix[: max(1, 64 - len(suffix))].rstrip("_")
...
#-------
    packnum = 0
    bot_username = getattr(app, "username", None) or BOT_USERNAME
    packname = _pack_short_name(message.from_user.id, bot_username)
    limit = 0
    try:
        while True:
