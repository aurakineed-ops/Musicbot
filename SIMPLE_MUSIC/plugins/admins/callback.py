...
async def close_stream_card(_, callback_query: CallbackQuery):
    """Delete the playback card only when pressed by a group admin."""
    try:
        if not await is_admin(callback_query):
            return await callback_query.answer(
                "Sirf group admins is button ko use kar sakte hain.",
                show_alert=True,
            )
        ...
        try:
            await callback_query.answer("Close nahi ho saka.", show_alert=True)
        except Exception:
            pass
                ...
                [
                    [
                        InlineKeyboardButton(
                            text=f"<emoji id='5440910346334255336'>👍</emoji> {get_upvotes}",
