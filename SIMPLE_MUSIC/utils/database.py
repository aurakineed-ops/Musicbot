...
async def is_welcome_enabled(chat_id: int) -> bool:
    if chat_id in welcome_state:
        return welcome_state[chat_id]
    setting = await welcomedb.find_one({"chat_id": chat_id})
    enabled = True if setting is None else bool(setting.get("enabled", True))
    welcome_state[chat_id] = enabled
    return enabled
...
async def get_welcome_message(chat_id: int):
    setting = await welcomedb.find_one({"chat_id": chat_id})
    return setting.get("custom") if setting else None


async def set_welcome_message(chat_id: int, content: dict):
    await welcomedb.update_one(
        {"chat_id": chat_id}, {"$set": {"custom": content}}, upsert=True
    )


async def clear_welcome_message(chat_id: int):
    await welcomedb.update_one(
        {"chat_id": chat_id}, {"$unset": {"custom": ""}}, upsert=True
    )
